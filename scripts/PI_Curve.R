...
#Project: Rim-Deep Transplant 2019
#Author: HM Putnam 
#Edited by: KH Wong
...
##### PI Curve Rate Calculation 20190822 #####
path.p<-"data/2019/PI_Curve_Data/20190822/" #the location of all your respirometry files 
...
colnames(Photo.R) <- c("Coral.ID","Intercept", "µmol.L.sec", "Temp")

#Load Sample meta info Info
Sample.Info <- read.csv(file="data/2019/PI_Curve_Data/20190822_PI_Curve_Sample_Info.csv", header=T) #read sample.info data
Sample.Info$Coral.ID <- paste(Sample.Info$Coral.ID,"_",Sample.Info$Light_Level,sep = "")
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:length(lt.levs)){    
    ...
    pdf(paste0("output/PI_Curves/",rename,"_",j,"thinning.pdf"))
    ...
    pdf(paste0("output/PI_Curves/",rename,"_",j,"regression.pdf"))
    plot(Regs)
    dev.off()
...
#view rates
Photo.R
write.csv(Photo.R,"output/20190822_PI_Curve_rates.csv")

#Merge rates with sample info
Data <- merge(Photo.R, Sample.Info, by="Coral.ID")

#calculate volume from height of water (measured height - end of ruler) and dimensions of tank, multiplied by the conversion of cubic inch to liters
Data$Water.Height.in <- Data$Water.Height.cm*0.393701
Data$Chamber.Vol.L <- (Data$Water.Height.in - 0.3125)*15.625*8.000*0.0163871 
...
write.csv(Data,"output/20190822_PI_Curve_rates_Pastreoides.csv")

#Remove blanks
Data <- read.csv("output/20190822_PI_Curve_rates_Pastreoides.csv")
Data <- Data %>%
  filter(Sample.Type != "Blank")
PI_curve <- ggplot(Data, aes(x = Light_Value, y = micromol.cm2.h, color = Sample.ID)) +
  geom_point() + 
  geom_line() +
...
ggsave(file="output/PI_Curve_20190822.pdf", PI_curve, width = 11, height = 11, units = c("in"))
