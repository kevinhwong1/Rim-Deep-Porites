#Title: Photosynthesis and Respiration Calculations for Rim/Deep Transplant
#Author: HM Putnam and NJ Silbiger
#Edited by: Kevin Wong
#Date Last Modified: 20190911
...
path.p<-"data/2019/Colony_Respirometry/20190825" #the location of all your respirometry files 
...
#Load Sample Info T1
Sample.Info <- read.csv(file="data/2019/Colony_Respirometry/20190825_Photo_Resp_Sample_Info.csv", header=T) #read in volume and sample.info data
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:length(lt.levs)){    
    ...
    pdf(paste0("output/2019/Colony_Photo_Resp_Output/",rename,"_",j,"thinning.pdf"))
    ...
    pdf(paste0("output/2019/Colony_Photo_Resp_Output/",rename,"_",j,"regression.pdf"))
    plot(Regs)
    dev.off()
...
write.csv(Photo.R, 'data/2019/Colony_Respirometry/2019_Photo.R.csv')

#Photo.R <- read.csv('data/July_Rim/Colony_Respirometry/Rim_T1_Photo.R.csv')#Split up the photostynthesis and respiration data into two dataframes
...
Resp$umol.sec <- Resp$umol.L.sec*Resp$Chamber.Vol.L
Photo$umol.sec <- Photo$umol.L.sec*Resp$Chamber.Vol.L
...
#Merging SA file
SA <- read.csv("data/2019/Color_Score/Color_Score.csv")
Resp2 <- merge(Resp, SA, by = "Fragment.ID")
Photo2 <- merge(Photo, SA, by = "Fragment.ID")

Resp2$umol.cm2.hr <- (Resp2$umol.sec.corr*3600)/Resp2$SA
Photo2$umol.cm2.hr <- (Photo2$umol.sec.corr*3600)/Photo2$SA
...
write.csv(Photo3, file="output/2019/Colony_Photo_Resp_Output/2019_Photosynthesis.rates.T1.csv")
write.csv(Resp3, file="output/2019/Colony_Photo_Resp_Output/2019_Respiration.rates.T1.csv")

#calculate gross photosynthesis Pnet -- Rdark

Photo3 <- read.csv("output/2019/Colony_Photo_Resp_Output/2019_Photosynthesis.rates.T1.csv")
Resp3 <- read.csv("output/2019/Colony_Photo_Resp_Output/2019_Respiration.rates.T1.csv")

resp.data <- merge(Photo3[,c(1,39)],Resp3[,c(1,39)], by="Fragment.ID")
...
write.csv(resp.data, file="output/2019/Colony_Photo_Resp_Output/2019_Respdata.csv")

## Attaching meta data 
meta <- read.csv("data/2019/Meta_Data/Master_Coral_Sheet.csv")

# Merging 
resp.data.meta <- merge(resp.data, meta, bby = "Fragment.ID")

#Calculate means
AllMeans <- ddply(resp.data.meta, c('Origin','Transplant.site'), summarize,
                  #pnet
...
#write Results
write.csv(resp.data.meta, file="data/2019/Colony_Respirometry/2019_AllRates.csv") # raw data
write.csv(AllMeans, file="data/2019/Colony_Respirometry/2019_AllMeans.csv") # Mean data

pd <- position_dodge(0.1) # moves object .05 to the left and right
legend.title <- "Treatment"

#PLot the means
Fig.Pn <-  ggplot(AllMeans, aes(x=Transplant.site, y=Pnet.mean,  color=Origin, group=Origin)) + #set up plot information
  geom_errorbar(aes(x=Transplant.site, ymax=Pnet.mean+Pnet.se, ymin=Pnet.mean-Pnet.se), colour="black", width=.1, position = pd) + #add standard error bars about the mean
...
Fig.Pg <-  ggplot(AllMeans, aes(x=Transplant.site, y=Pgross.mean,  color=Origin, group=Origin)) + #set up plot information
  geom_errorbar(aes(x=Transplant.site, ymax=Pgross.mean+Pgross.se, ymin=Pgross.mean-Pgross.se), colour="black", width=.1, position = pd) + #add standard error bars about the mean
  geom_line(position=pd, color="black")+
  xlab("Transplant Site") + #label x axis
  ylab(expression("Oxygen Flux " (mu~mol ~ cm^{-2} ~ h^{-1}))) + #label y axis
  #  ylim(0.3, 1.3)+
...
Fig.Rd <-  ggplot(AllMeans, aes(x=Transplant.site, y=Rdark.mean,  color=Origin, group=Origin)) + #set up plot information
  geom_errorbar(aes(x=Transplant.site, ymax=Rdark.mean+Rdark.se, ymin=Rdark.mean-Rdark.se), colour="black", width=.1, position = pd) + #add standard error bars about the mean
  geom_line(position=pd, color="black")+
  xlab("Transplant Site") + #label x axis
  ylab(expression("Oxygen Flux " (mu~mol ~ cm^{-2} ~ h^{-1}))) + #label y axis
  #  ylim(-0.9, -0.1)+
...
ggsave(file="output/2019/2019_ColonyRespirometry.pdf", Resp.Figs, width = 11, height = 6, units = c("in"))
